...
export function OrdersTable({ orders, drivers, eventTypes }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Search bar - outside the table card */}
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          <IconSearch className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-zinc-500" />
          <input
            type="text"
            placeholder="Suche nach Kunde, Firma, Location, Ort, Event, Auftragsnummer..."
            value={search}
...
function MonthGroup({
  ...
  return (
    <div className="border-b border-white/[0.04] last:border-b-0">
      ...
      {!collapsed && <OrderTable orders={orders} onRowClick={onRowClick} />}
    </div>
  );
}
        ...
        {orders.map((order) => {
          ...
          return (
            ...
            >
              ...
              <TableCell className="text-zinc-400 text-sm max-w-[140px]">
                {firma ? (
              ...
              <TableCell className="text-zinc-400 text-sm max-w-[100px]">
                {ort ? (

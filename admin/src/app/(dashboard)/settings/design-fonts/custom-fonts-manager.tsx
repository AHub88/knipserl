  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        />
        <IconUpload className="size-10 text-white/30 mx-auto mb-3" />
        <p className="text-sm text-white/60">
          {uploading ? (
            "Wird hochgeladen..."
          ) : (
            <>
              <span className="text-[#F6A11C] font-medium">Schriftdateien auswählen</span> oder hier hinziehen
            </>
          )}
        </p>
    ...
    <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-semibold text-white/90">{family}</h3>
          <p
            className="text-2xl mt-1 text-white/70 truncate"
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {variants.map((v) => (
              <span
                key={v.id}
                className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-md bg-white/5 text-white/50"
              >
        ...
        {variants.length === 1 && (
          <button
            ...
            className="shrink-0 p-2 rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors"

...
export default async function CustomerDetailPage({
  ...
  const sc: Record<string, string> = {
    DRAFT: "bg-zinc-500/15 text-zinc-400", SENT: "bg-blue-500/15 text-blue-400",
    ...
    CANCELLED: "bg-zinc-500/15 text-zinc-400", OPEN: "bg-amber-500/15 text-amber-400",
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="rounded-2xl border border-white/[0.10] bg-card shadow-lg shadow-black/30 p-4 sm:p-6">
        <div className="flex items-center gap-2 sm:gap-3 mb-4">
          <Link href="/customers" className="flex items-center justify-center size-9 rounded-lg border border-white/[0.08] bg-[#1c1d20] text-zinc-400 hover:text-zinc-200 transition-colors shrink-0">
            <IconArrowLeft className="size-4" />
          </Link>
          <span className="text-xs text-muted-foreground">Kunde</span>
          <span className={"ml-auto inline-flex items-center rounded-md px-2.5 py-1 text-xs font-semibold border " + (customer.customerType === "BUSINESS" ? "bg-blue-500/15 text-blue-400 border-blue-500/25" : "bg-zinc-500/15 text-zinc-400 border-zinc-500/25")}>
            {customer.customerType === "BUSINESS" ? "Geschäftskunde" : "Privatkunde"}
          </span>
        </div>

        <h1 className="text-xl sm:text-3xl font-bold text-zinc-100 mb-3">{customer.name}</h1>

        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
          {customer.email && <span className="flex items-center gap-1.5 text-zinc-400"><IconMail className="size-3.5" />{customer.email}</span>}
          {customer.phone && <span className="flex items-center gap-1.5 text-zinc-400"><IconPhone className="size-3.5" />{customer.phone}</span>}
          {customer.company && <span className="flex items-center gap-1.5 text-zinc-400"><IconBriefcase className="size-3.5" />{customer.company}</span>}
        </div>

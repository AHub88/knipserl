...
function ClaimButton({ orderId }: { orderId: string }) {
  ...
  return (
    <button
      onClick={handleClaim}
      disabled={loading}
      className="w-full py-2.5 rounded-lg bg-[#F6A11C] text-black text-sm font-bold active:scale-[0.98] disabled:opacity-50 transition-all"
    >
...
export function MyOrdersTabs({
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: "assigned", label: "Zugewiesen", count: assignedOrders.length },
    { key: "open", label: "Offen", count: openOrders.length },
        ...
        openOrders.length === 0 ? (
          ...
          <div className="space-y-2">
            {openOrders.map((order) => (
              <details key={order.id} className="group rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                <summary className="flex items-center gap-3 p-3.5 cursor-pointer active:bg-white/[0.03] transition-colors list-none [&::-webkit-details-marker]:hidden">
                  <DateBadge iso={order.eventDate} />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-zinc-200 truncate">{order.locationName}</div>
                    <div className="text-[11px] text-zinc-500">{order.eventType} · {formatDate(order.eventDate)}</div>
                  </div>
                  {order.compensation > 0 && (
                    <span className="text-xs font-bold text-emerald-400 tabular-nums shrink-0">{order.compensation.toFixed(0)} €</span>
                  )}
                  <svg className="size-4 text-zinc-600 shrink-0 transition-transform group-open:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
                </summary>
                <div className="px-3.5 pb-3.5 pt-2 space-y-3 border-t border-white/[0.04]">
                  <div className="space-y-2 text-xs">
                    <div>
                      <span className="text-zinc-500">Kunde</span>
                      <div className="text-zinc-300 font-medium">{order.customerName}</div>
                    </div>
                    <div>
                      <span className="text-zinc-500">Adresse</span>
                      <div className="text-zinc-300">{order.locationAddress}</div>
                    </div>
                  </div>
                  {order.extras.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {order.extras.map((e) => (
                        <span key={e} className="px-1.5 py-0.5 rounded border border-white/[0.08] text-[10px] text-zinc-400">{e}</span>
                      ))}
                    </div>
                  )}
                  {order.notes && (
                    <div className="text-[11px] text-zinc-500 bg-white/[0.03] rounded-lg px-3 py-2">{order.notes}</div>
                  )}
                  <DeliveryInfo order={order} />
                  <div className="flex gap-2">
                    <Link
                      href={`/orders/${order.id}`}
                      className="flex-1 py-2.5 rounded-lg bg-white/[0.06] text-center text-xs font-semibold text-zinc-300 active:bg-white/[0.1] transition-colors"
                    >
                      Details
                    </Link>
